...
#include "mex.h"
/* "MEX-files are a way to call your custom C, C++ or FORTRAN routines 
   directly from MATLAB as if they were MATLAB built-in functions"
...
// coordinates for {0,20,40,60,80,120,140,160} degrees
double uu[9] = {1.0000, 
	0.9397, 
	0.7660, 
	0.500, 
...
mxArray *process(const mxArray *mximage, const mxArray *mxsbin) {
	...
	/* Check if @mximage is a color image
	   (3d array, with last dim of size 3 and elements of type double)  */
	if (mxGetNumberOfDimensions(mximage) != 3 ||
			dims[2] != 3 ||
			mxGetClassID(mximage) != mxDOUBLE_CLASS)
		mexErrMsgTxt("Invalid input");

	int sbin = (int)mxGetScalar(mxsbin); // cell size
	...
	for (int x = 1; x < visible[1]-1; x++) {
		for (int y = 1; y < visible[0]-1; y++) {
			...
			/* NOTE: matlab stores arrays in column-major order, while images are indexed
			   with (i,j) = (y,x) as a result the color channel index gets the biggest
			   pointer increment, then goes x coordinate, and y index increments by 1. */

			/* (?) Seems like the border dims[*]-2 pixels are processed repeatedly in place
			   of those pixels outside the image in truncated cells
			   y = min(y, dims[0]-2);
			   x = min(x, dims[1]-2);
			   s = &im[y,x,0];
			 */ 
